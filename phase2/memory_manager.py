...
class MemoryManager:
    def __init__(self, agent_name: str, base_path: str = "base_agents"):
        ...
        # Reduce startup output for production
        print(f"✅ Memory Manager initialized for {self.agent_name}")
        if self.short_term_memory:
            print(f"   Loaded {len(self.short_term_memory)} short-term memory entries")
    ...
    def _load_conscious_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"Warning: Error loading conscious memory: {e}")
            return {"memories": [], "relationships": [], "persona": {}}
    ...
    def _load_unconscious_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"Warning: Error loading unconscious memory: {e}")
            return {
                "signifiers": [], "signifying_chains": [], "object_a": {}, "symptom": {},
                "structural_positions": [], "fantasy_formula": "", "jouissance_economy": {}, "dream_work_patterns": {}
            }
    ...
    def retrieve_memories(self, query: str, n_results: int = 5) -> List[Dict[str, Any]]:
        """Retrieve relevant memories using RAG system with relaxed threshold."""
        # Use lower threshold for better interaction (0.1 instead of 0.2)
        return self.rag_system.search_memories(query, n_results, similarity_threshold=0.1)
    
    def retrieve_relationships(self, query: str, n_results: int = 3) -> List[Dict[str, Any]]:
        """Retrieve relevant relationships using RAG system with relaxed threshold."""
        # Use lower threshold for better interaction (0.1 instead of 0.2)
        return self.rag_system.search_relationships(query, n_results, similarity_threshold=0.1)
